  ...
  generate:
    ...
    - name: Generate Build Matrix
      ...
      with: 
        root: .
        parser: dockerbuild
        flags: "--registry ghcr.io/rse-radiuss"
...
For docker build (and other parsers) it's suggested to have a check for if the matrix
is empty to know to continue or not.
...
| flags | Any additional flags that are needed by the parser | false | unset |
